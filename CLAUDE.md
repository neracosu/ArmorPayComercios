...
**La caja ve solo lo suyo (2026-09-18).** Un `OPERATOR` lista únicamente lo que él mismo cobró o consultó (`userId` de la sesión): nunca lo que entró a las cuentas del comercio, los totales del día ni lo de otras cajas — eso es del `ORG_ADMIN`, el «master» del comercio. **El aislamiento por tenant NO cubre esto**: caja y dueño comparten `organizationId`, la extensión de Prisma deja pasar todo, y esta frontera va a mano, en el servidor, en cada página y cada `route.ts` (ocultar el enlace del menú no es un gate). Lección: `/pagos` nació el 09-16 «para caja y dueño por igual» y durante dos días cada caja pudo ver la facturación completa del comercio, con totales y CSV; lo reportó un comercio. Al crear una pantalla con datos de todo el comercio: primero el gate `role !== "ORG_ADMIN"`, comparando contra el dueño y no contra la caja para que un rol nuevo nazca del lado seguro; para la caja, `listarCobros({ userId })`. Tampoco se le muestra a la caja la lista de pagos sin cobrar: con la referencia completa a la vista puede cobrar un pago ajeno contra una venta en efectivo — para el «ya pagué y no aparece» está el buscador de `/validar`, que exige el final de la referencia.
...
- **Un solo cliente Prisma por proceso.** Nunca `new PrismaClient()` fuera de `src/lib/prisma.ts`: para leer sin tenant (login, registro, `/plataforma`) se usa `prismaSinTenant`, que va sobre el mismo pool. En `globalThis` se comparte SOLO el cliente base, **nunca el extendido**: Next carga `tenant-context.ts` una vez por capa del build (las server actions van en otra), y un cliente extendido reusado de otra capa busca el contexto en un `AsyncLocalStorage` ajeno → «sin contexto de tenant» en cada acción (regresión del mismo 2026-09-15, arreglada a la hora; `test-isolation.ts` NO la detecta porque tsx carga un solo grafo de módulos, así que tras tocar `prisma.ts` hay que probar a mano una acción real con sesión de comercio). Cada cliente suelto abre hasta 25 conexiones y el MariaDB (tope 151) lo comparten ~15 apps del servidor, incluido el interno. El 2026-09-15 había 16 clientes, el servidor llegó a 149/151 y un comercio recién registrado no pudo entrar (`1040 Too many connections`).
